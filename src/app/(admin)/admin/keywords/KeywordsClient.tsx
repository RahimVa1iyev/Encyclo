...
import { addKeyword, removeKeyword, addCategory, generateKeywordsAI } from "./actions"
...
export default function KeywordsClient({ categories }: { categories: Category[] }) {
  ...
  const emptyCategories = categories.filter(c => c.category_keywords.length === 0)

  const sortedCategories = [...categories].sort((a, b) => {
    return a.category_keywords.length - b.category_keywords.length
  })
...
function CategoryCard({ category }: { category: Category }) {
  ...
  const keywordCount = category.category_keywords.length

  const handleGenerateAI = async () => {
    try {
      setIsLoadingAI(true)
      const existing = category.category_keywords.map(k => k.keyword)
      ...
      <div style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {keywordCount === 0 ? (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {category.category_keywords.sort((a, b) => b.weight - a.weight).map(k => (
