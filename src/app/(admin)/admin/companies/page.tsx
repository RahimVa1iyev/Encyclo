...
export default async function AdminCompaniesPage() {
  ...
  const { data: companies, error } = await supabase
    .from('companies')
    .select('id, slug, status, created_at, company_translations(name), subscriptions(plan, status)')
  ...
  const draft = safeCompanies.filter(c => c.status === 'draft').length
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {safeCompanies.map((company: any) => {
                const compTrans = Array.isArray(company.company_translations) ? company.company_translations[0] : company.company_translations
