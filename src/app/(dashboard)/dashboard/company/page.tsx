...
export default function CompanyProfilePage() {
    ...
    const fetchData = async () => {
      ...
      if (company) {
        ...
        if (socialData) {
          ...
          socialData.forEach(s => { links[s.platform] = s.url; });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const platformsToSave = Object.entries(socialLinks).filter(([, url]) => url.trim());
      if (platformsToSave.length > 0) {
        const { error: socialError } = await supabase
          .from('company_social_links')
          .upsert(
            platformsToSave.map(([platform, url]) => ({ 
              company_id: companyId, 
              platform, 
              url: url.trim() 
            })),
      ...
      const emptyPlatforms = Object.entries(socialLinks)
        .filter(([, url]) => !url.trim())
        .map(([p]) => p);
            ...
            <Card>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="rounded-lg border p-3" style={{ borderColor: "var(--border)", backgroundColor: "var(--muted)" }}>
                  <div className="text-[10px] text-muted-foreground truncate">encyclo.az › companies › {companySlug || 'sirket'}</div>
