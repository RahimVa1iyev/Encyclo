...
export default async function AdminDashboardPage() {
  ...
  const [totalCompanies, activeSites, totalKeywords] = await Promise.all([
    ...
    prisma.partnerSite.count({ where: { status: "active" } }),
    prisma.categoryKeyword.count(),
  ]);

  // Fetch recent partner sites
  const recentSites = await prisma.partnerSite.findMany({
    select: { id: true, name: true, domain: true, status: true, created_at: true },
