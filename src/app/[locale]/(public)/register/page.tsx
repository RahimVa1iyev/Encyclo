...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Log the user in with NextAuth so the server action has a valid session
      ...
      if (signInResult?.error) {
         throw new Error("Hesab yaradıldı, lakin giriş etmək mümkün olmadı.");
      }

      // 3. Create Company & Translation safely
      try {
        await createCompanyAfterRegisterAction(companySlug, companyName);
      } catch (err: unknown) {
        throw new Error(
