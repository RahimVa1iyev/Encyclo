...
export default function AIContentPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const { getAIContentProductsData } = await import('../../actions');
        const data = await getAIContentProductsData();
        if (data) {
  ...
  const handleApply = async () => {
    ...
    setIsSaving(true);
    try {
      const { updateProductTranslationDescAction } = await import('../../actions');
